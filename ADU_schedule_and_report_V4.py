...
P = ['Holiday Movies (Prime)', 'ION Originals (Prime)', 'Prime', 'Prime no CM']
NP = ['Daytime (M-F)', 'Early Morning (M-S)', 'Fringe (M-S)', 'Holiday Movies (Non Prime)', \
...
def get_dict(df, ratings, endq):
    ...
    for col, row in df.iterrows():
        if date_comparison(row['Week Start Date'], endq):
            ...
            ''' if not row['Guarantee Name']:
                changeDF[0].append(row['Deal Name'])
            else:
                changeDF[0].append(row['Guarantee Name'])

            if dayparts(row) == 'P':
                changeDF[1].append('P')
            else:
                changeDF[1].append('NP')'''
        else:
            continue
...
def newdata_to_df(df, general, output):
    ...
    combined = pd.merge(combined, imp_df)
...
def get_summary(report_values, date_string, quar):
    ...
    wb = load_workbook(filename = 'Summary.xlsx')
    ws = wb["Sheet1"]
...
def main(Q_num = 2):
    ...
    zf = zipfile.ZipFile('Dealmaker BI weekly reports.zip') 
    ...
    ratings_file = '2019-07-17 ION Q3 & Q4 Internal Estimates.xlsx'
        
    
    quarters = pd.read_csv('timeList.csv')
